defmodule Encryptor.Ecto.ScopeTest do
  use ExUnit.Case, async: true

  alias Encryptor.Ecto.Scope

  doctest Encryptor.Ecto.Scope
  ...
  describe "put/1 and get/0" do
    ...
    test "put/1 refuses a non-binary scope" do
      assert_raise FunctionClauseError, fn -> Scope.put(:merchant_7f3) end
    end
  end

  describe "fetch!/0" do
    # sabotage: made fetch!/0 return {:ok, scope} -> red.
    test "returns the scope set" do
      :ok = Scope.put("merchant_7f3")

      assert "merchant_7f3" = Scope.fetch!()
    end

    # sabotage: made fetch!/0 return nil for an empty scope instead of raising -> red.
    test "raises when no scope is set, naming the calls that set one" do
      error = assert_raise RuntimeError, fn -> Scope.fetch!() end

      assert error.message =~ "no scope set"
      assert error.message =~ "Encryptor.Ecto.Scope.put/1"
      assert error.message =~ "Encryptor.Ecto.Scope.wrap/2"
    end
  end

  describe "clear/0" do
    # sabotage: made clear/0 a no-op returning :ok -> red.
    test "removes the scope from scope" do
      :ok = Scope.put("merchant_7f3")

      assert :ok = Scope.clear()
      assert :error = Scope.get()
    end

    # sabotage: made clear/0 raise when nothing was set -> red.
    test "is :ok when nothing was set" do
      assert :ok = Scope.clear()
    end
  end
  ...
  describe "wrap/2" do
    ...
    test "runs the function with the scope set and returns its value" do
      assert "merchant_7f3" = Scope.wrap("merchant_7f3", &Scope.fetch!/0)
    end
    ...
    test "restores the previous scope rather than clearing it" do
      :ok = Scope.put("merchant_7f3")
    ...
    test "restores an empty scope to empty rather than leaving the wrapped scope" do
      assert :error = Scope.get()
      assert "merchant_7f3" = Scope.wrap("merchant_7f3", &Scope.fetch!/0)
      assert :error = Scope.get()
    end
    ...
    test "restores the previous scope when the function raises" do
      :ok = Scope.put("merchant_7f3")

      assert_raise ArgumentError, fn ->
        Scope.wrap("merchant_a19", fn -> raise ArgumentError, "signup wizard variant blew up" end)
      end
    ...
    test "nests" do
      result =
        Scope.wrap("merchant_7f3", fn ->
          inner = Scope.wrap("merchant_a19", &Scope.fetch!/0)
          {inner, Scope.fetch!()}
        end)
    ...
    test "refuses a function of the wrong arity" do
      assert_raise FunctionClauseError, fn ->
        Scope.wrap("merchant_7f3", fn _scope -> :never_run end)
      end
    end
  end
  ...
  describe "process scope" do
    ...
    test "is carried across a Task boundary by fetching and wrapping explicitly" do
      ...
      task = Task.async(fn -> Scope.wrap(scope, &Scope.get/0) end)
    ...
    test "is independent between concurrent processes" do
      ...
      other = Task.async(fn -> Scope.wrap("merchant_a19", &Scope.fetch!/0) end)
