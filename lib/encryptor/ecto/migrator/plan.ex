defmodule Encryptor.Ecto.Migrator.Plan do
  ...
  how the scope is resolved for its rows, and the fields to rewrite. The
  ...
  ## What the scope field means here

  ADR-0002 decision 3: the migrator supplies the scope explicitly, per row,
  rather than reading an ambient scope. So a rewrite's `:scope` says where
  the *migrator* finds it:

    * `{:column, :account_id}` - read it off the row being rewritten, which is
      `scope_from :account_id` in the DSL.
    * `:none` - the field is global; there is no scope to resolve.
    * a module - an `Encryptor.Ecto.ScopeContext` implementation the migrator
      asks, for a host whose scope is not a column of the table being
      rewritten.

  `:process` is deliberately absent, and the DSL refuses it: process scope is
  ...
  @typedoc """
  How the migrator resolves the scope for the rows of one rewrite.

  See the moduledoc for why `:process` is not one of them.
  """
  @type scope :: {:column, atom()} | :none | module()

  @typedoc """
  One schema's rewrite: its scope strategy and its fields, in declared order.
